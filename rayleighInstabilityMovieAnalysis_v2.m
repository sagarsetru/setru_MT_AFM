...
% if picking 2 or 3 line profiles and stitching together
% (for curved MTs)
n_lines_to_stich = 4;
...
if n_lines_to_stich > 1
    
    for i = 1:n_lines_to_stich
        ...
        cx1_temp = zeros(size(img2,3),size(ctest,1));
        cy1_temp = zeros(size(img2,3),size(ctest,1));
        c_temp = zeros(size(img2,3),size(ctest,1));

        for j = 1:size(img2,3)
            [cx1_temp(j,:),cy1_temp(j,:),c_temp(j,:)] = improfile(img2(:,:,j),xi,yi);
            %c(j,:) = c(j,:)-min(c(j,:));
